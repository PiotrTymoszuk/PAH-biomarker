---
title: "Risk stratification and phenotyping in pulmonary arterial hypertension with machine learning algorithms and cluster analysis – a long-term retrospective multicenter trial"
...
Differences in survival between the participant clusters or participants stratified by risk score tertiles were compared by Kaplan-Meier (KM) analysis, Mentel-Haenszel or log-rank test [@Harrington1982; @Therneau2000]. KM analysis and visualization of its results were done with tools provided by _survival_ and _survminer_ packages [@Kassambara2016].
...
suppl_tables$study_vars %>% 
  ...
  footnote(1, 1, part = 'header', value = as_paragraph('Variable name in the R analysis pipeline.'), ref_symbols = 'a') %>% 
  footnote(1, 3, part = 'header', value = as_paragraph('Variable name in the figures and tables.'), ref_symbols = 'b') %>% 
  theme_vanilla
...
```{r tab-cohort, tab.cap = 'Supplementary characteristic of the study cohorts.'}
...
suppl_tables$cohort_features %>% 
  ...
  footnote(c(2, 4:15), 4, part = 'body', value = as_paragraph('Mann-Whitney U test.'), ref_symbols = 'a') %>% 
  footnote(c(2, 4:15), 5, part = 'body', value = as_paragraph('Wilcoxon r effect size statistic.'), ref_symbols = 'b') %>% 
  footnote(c(3, 16, 17), 4, part = 'body', value = as_paragraph('\u03C7\u00B2 test.'), ref_symbols = 'c') %>% 
  footnote(c(3, 16, 17), 5, part = 'body', value = as_paragraph('Cramer V effect size statistic.'), ref_symbols = 'd') %>% 
  theme_vanilla
...
suppl_tables$univariable_cox %>% 
  ...
  merge_v(1:2) %>% 
  footnote(1, 5, part = 'header', value = as_paragraph('Hazard ratio with 95% confidence interval.'), ref_symbols = 'a') %>% 
  footnote(1, 7, part = 'header', value = as_paragraph('Concordance index with 95% confidence interval.'), ref_symbols = 'b') %>%
  theme_vanilla
...
suppl_tables$clust_IBK %>% 
  ...
  width(4, width = 2.5, unit = 'cm') %>% 
  width(5, width = 2, unit = 'cm') %>% 
  footnote(c(2:3, 5:6, 8, 10:13, 15:17, 19, 22:26), 4, part = 'body', value = as_paragraph('Mann-Whitney U test.'), ref_symbols = 'a') %>% 
  footnote(c(2:3, 5:6, 8, 10:13, 15:17, 19, 22:26), 5, part = 'body', value = as_paragraph('r effect size statistic.'), ref_symbols = 'b') %>% 
  footnote(c(4, 7, 9, 14, 18, 20:21), 4, part = 'body', value = as_paragraph('\u03C7\u00B2 test.'), ref_symbols = 'c') %>% 
  footnote(c(4, 7, 9, 14, 18, 20:21), 5, part = 'body', value = as_paragraph('Cramer V effect size statistic.'), ref_symbols = 'd') %>% 
  theme_vanilla
...
```{r fig-uni-cox, fig.width = mm_inch(suppl_figures$uni_cox$w), fig.height = mm_inch(suppl_figures$uni_cox$h), fig.cap = 'Univariable Cox proportional hazard modeling.'}
...
```{r fig-cluster-qc, fig.width = mm_inch(suppl_figures$cluster_qc$w), fig.height = mm_inch(suppl_figures$cluster_qc$h), fig.cap = 'Development of participant clusters.'}
...
Clustering of the training Innsbruck (IBK) cohort participants in respect to the survival-associated factors identified by elastic-net modeling (__Figure 2A__) was investigated by PAM (partitioning around medoids) algorithm and cosine distance measure.
...
```{r fig-clust-diff, fig.width = mm_inch(suppl_figures$clust_diff$w), fig.height = mm_inch(suppl_figures$clust_diff$h), fig.cap = 'Differences in study variables between the participant clusters.'}

suppl_figures$clust_diff$plot

```

__Supplementary Figure S\@ref(fig:fig-clust-diff). Differences in study variables between the participant clusters.__

Training Innsbruck (IBK) cohort participants were clustered as presented in __Figure 4__ and __Supplementary Figure S\@ref(fig:fig-cluster-qc)__. Cluster assignment of the test Linz/Vienna (LZ/W) cohort participants was accomplished by k-nearest neighbor label propagation procedure. Differences in the study variables (__Supplementary Table S\@ref(tab:tab-study-variables)__) between the clusters were determined by Mann-Whitney test with r effect size statistic or by $\chi^2$ test with Cramer V effect size statistic for numeric and categorical features, respectively. P values were adjusted for multiple testing with Benjamini-Hochberg method (pFDR). 
